...
func (m Model) View() string {
	if m.finished {
		switch m.EchoMode {
		case EchoNormal:
			return FontColor(m.ValidateOkPrefix, colorValidateOk) + " " + m.Prompt + m.Value() + "\n"
		case EchoNone:
			return FontColor(m.ValidateOkPrefix, colorValidateOk) + " " + m.Prompt + "\n"
		case EchoPassword:
			return FontColor(m.ValidateOkPrefix, colorValidateOk) + " " + m.Prompt + GenMask(len([]rune(m.Value()))) + "\n"
		}
	}
	...
	if m.err != nil {
		...
		prompt = FontColor(m.ValidateOkPrefix, colorValidateOk) + " " + m.input.View()
	}
